...
class ParticleFilter(object):
    ...
    def __init__(self, dim_x, n_particles=100, tau=0.5, forward=None, likelihood=None):
        ...
        self.particles = np.zeros((n_particles,6))
    ...
    def resample(self, tau=None):
        ...
        N_eff = 1/(np.sum(np.sum(np.square(self.weights))))
        
        # resample if necessary
        if N_eff <= threshold:
            # Hint: you can use numpy.random.choice
            index = np.random.choice(self.n_particles, self.n_particles, replace=False)
    ...
    def update(self, z, likelihood=None):
        ...
        self.weights = np.multiply(self.weights,likelihoods)
        self.weights = self.weights/np.sum(np.sum(self.weights))
